...
use actix_files::{Directory, FilesDirs, FilesService};
use actix_service::{IntoServiceFactory, ServiceFactory};
use actix_web::{
    dev::{AppService, HttpServiceFactory, RequestHead, ServiceRequest, ServiceResponse},
    error::Error,
    guard::Guard,
...
use paperclip_core::v2::models::{
    DefaultOperationRaw, DefaultResponseRaw, DefaultSchemaRaw, HttpMethod, SecurityScheme,
};
...
impl Files {
    ...
    pub fn new<T>(mount_path: &str, serve_from: T) -> Self
    where
        FilesDirs: std::convert::From<T>,
    {
        Self::new_tagged(mount_path, serve_from, vec![])
    }

    pub fn new_tagged<T>(mount_path: &str, serve_from: T, tags: Vec<String>) -> Self
    where
        FilesDirs: std::convert::From<T>,
    {
        ...
        BTreeMap::from([(
            ...
            DefaultOperationRaw {
                ...
                responses: BTreeMap::from([(
                    "200".to_string(),
                    paperclip_core::v2::models::Either::Right(DefaultResponseRaw::default()),
                )]),
                parameters: vec![],
                deprecated: false,
